    ...
    if (commandName === "topme") {
      try {
        ...
        if (!discordUserId) {
          return new Response(JSON.stringify({
            type: 4,
            data: { content: "❌ Discord-ID konnte nicht ermittelt werden.", flags: 64 },
          }), { headers: { "Content-Type": "application/json" } });
        }
        ...
        if (!profiles || profiles.length === 0) {
          return new Response(JSON.stringify({
            type: 4,
            data: { content: "❌ Dein Discord-Account ist nicht mit einem Profil verknüpft. Bitte trage deine Discord-ID in deinem Profil auf der Webseite ein.", flags: 64 },
          }), { headers: { "Content-Type": "application/json" } });
        }
