...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let query = supabase.from("discord_embeds").select("*");
    ...
    for (const row of rows ?? []) {
      const embeds = Array.isArray(row.embeds) ? row.embeds : [row.embeds];
      ...
      if (msgId) {
        ...
        if (res.status === 404) {
          // Alte Nachricht existiert nicht mehr → neu senden
          msgId = null;
        }
      }
