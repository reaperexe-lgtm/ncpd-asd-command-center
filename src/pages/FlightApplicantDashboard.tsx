...
import { CheckCircle, Circle, Plane, LogOut, BookOpen, Clock, Phone, Copy } from "lucide-react";
...
const FlightApplicantDashboard = () => {
  ...
  // Only show modules in the "Fluglizenz" category
  const { data: modules } = useQuery({
    queryKey: ["flight-training-modules"],
    queryFn: async () => {
  ...
  const { data: progress } = useQuery({
    queryKey: ["flight-applicant-progress", user?.id],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const { data: trainerContacts } = useQuery({
    queryKey: ["trainer-contacts-flight-applicant"],
    queryFn: async () => {
  ...
  const completedCount = progress?.filter((p) => p.completed && modules?.some((m) => m.id === p.module_id)).length ?? 0;
  const totalCount = modules?.length ?? 0;
  const progressPercent = totalCount > 0 ? (completedCount / totalCount) * 100 : 0;

  const getProgressForModule = (moduleId: string) => progress?.find((p) => p.module_id === moduleId);
      ...
      <div className="border-b border-border bg-card">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-foreground">Fluglizenz Ausbildung</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="border border-border rounded-xl bg-card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-lg font-semibold text-foreground">Dein Fluglizenz-Fortschritt</h2>
          </div>
          ...
          {totalCount === 0 && (
            <p className="text-xs text-muted-foreground mt-2">
              Noch keine Fluglizenz-Module konfiguriert. Wende dich an einen Ausbilder.
            </p>
          )}
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="fortschritt" className="mt-6">
            {totalCount === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                Noch keine Fluglizenz-Module konfiguriert.
              </div>
            ) : (
          ...
          <TabsContent value="leitfaden" className="mt-6">
            <LeitfadenContent
              ...
              onNavigateToTheoriepruefung={() => setActiveTab("theorie")}
            />
          </TabsContent>
...
export default FlightApplicantDashboard;